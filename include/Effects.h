...
// #define PIXEL_PIN PB6                            // Пин для подключения
...
enum effects_status { off, alarm, on = 250 };
...
class Effect {
 ...
 public:
  ...
  Effect(double _speed_coeff = 0, uint8_t _scale = 0)
      : scale(_scale), speed_coeff(_speed_coeff) {}
  ...
  virtual void Set_Scale(uint8_t _scale) { scale = _scale; }
...
class Color : public Effect {
 public:
  Color()
      : Effect(COLOR_C)  // TO-DO Возможна ошибка
  {}
...
class ColorALARM : public Effect {
 ...
 public:
  ...
  void Set_Scale(uint8_t _scale) override { step = _scale * 256 / (speed + 1); }
 ...
 public:
  ...
  void Draw() override {
    ...
    for (int i = 0; i < strip.numPixels(); i++) {  // For each pixel in strip...
      ...
      int pixelHue =
          firstPixelHue + (i * 65536L / strip.numPixels() * 64 / (1 + scale));
...
class Perlin : virtual public Effect {
 ...
 public:
  Perlin()
      : Effect(PERLIN_C, PERLIN_S)  // Effect(255, 3)
  {}
  ...
  void Draw() override {
    ...
    for (uint8_t y = 0; y < PIXEL_HEIGHT; ++y) {
      for (uint8_t x = 0; x < PIXEL_WIDTH; ++x) {
        // auto const noise = (db::perlin(double(x) * fx, double(y) * fy, step *
        // 0.25) * 1.0 +
        //                     db::perlin(double(x) * fx, double(y) * fy, step *
        //                     0.75) * 0.5) /
        //                    1.5;
        auto const noise =
            db::perlin(double(x) * fx, double(y) * fy, step * speed_coeff);

        uint32_t chanels =
            this->func(noise);  // TO-DO Возможна ошибка, внимание на RGBW
        strip.setPixelColor(x * 16 + (x % 2) * 15 + y - 2 * y * (x % 2),
                            chanels);
      }
    }
...
class Fire : public Effect {
  ...
  /* This is the map of colors from coolest (black) to hottest. Want blue
   * flames? Go for it! */
  const uint32_t colors[11] = {0x000000, 0x100000, 0x300000, 0x600000,
                               0x800000, 0xA00000, 0xC02000, 0xFF2800,
                               0xFF3600, 0xFF4400, 0xFF5210};
  // 0xFFC030};
  // 0xC08030};
  ...
  /* Flare constants */
  const uint8_t flarerows =
      2; /* number of rows (from bottom) allowed to flare */
  const static uint8_t maxflare = 8; /* max number of simultaneous flares */
  const uint8_t flarechance =
      50; /* chance (%) of a new flare (if there's room) */
 ...
 public:
  ...
  uint32_t isqrt(uint32_t n) {
    ...
    return (largeCandidate * largeCandidate > n) ? smallCandidate
                                                 : largeCandidate;
  }
    ...
    for (int i = (y - b); i < (y + b); ++i) {
      for (int j = (x - b); j < (x + b); ++j) {
        if (i >= 0 && j >= 0 && i < PIXEL_WIDTH && j < PIXEL_HEIGHT) {
          int d =
              (flaredecay * isqrt((x - j) * (x - j) + (y - i) * (y - i)) + 5) /
              10;
          uint8_t n = 0;
          if (z > d) n = z - d;
          if (n > buffer[i][j]) {  // can only get brighter
            buffer[i][j] = n;
          }
        }
      }
  ...
  void Draw() override {
    ...
    for (i = PIXEL_WIDTH - 1; i > 0; --i) {
      for (j = 0; j < PIXEL_HEIGHT; ++j) {
        uint8_t n = 0;
        if (buffer[i - 1][j] > 0) n = buffer[i - 1][j] - 1;
        buffer[i][j] = n;
      }
    }
    ...
    for (i = 0; i < PIXEL_WIDTH; ++i) {
      for (j = 0; j < PIXEL_HEIGHT; ++j) {
        strip.setPixelColor(i * 16 + (i % 2) * 15 + j - 2 * j * (i % 2),
                            colors[buffer[i][j]]);
      }
    }
    strip.show();
...
#define NUMBER_OF_EFFECTS 9
  Effect *effect[NUMBER_OF_EFFECTS] = {
      new ColorALARM(),   new Color(), new Rainbow(),
      new Clean_Perlin(), new Fire(),  new Sky(),
      new Lava(),         new Ocean(), new Forest()};
 ...
 public:
  ...
  void Run() {
    if (working_status == effects_status::off) return;
  ...
  void Set_Effect_Prev() {
    if (working_status == effects_status::off) return;
    if (current_effect_number > 0) Set_Effect(current_effect_number - 1);
  }
  ...
  void Set_Brigtness_Next() {
    if (working_status == effects_status::off) return;
    if (brightness < 255) Set_Brigtness(brightness + 1);
  }

  void Set_Brigtness_Prev() {
    if (working_status == effects_status::off) return;
    if (brightness > 0) Set_Brigtness(brightness - 1);
  }
