...
enum delay_time { repeat_delay_time = 25, pause_time = 150, long_time = 250 };
...
class Virtual_Button {
 ...
 public:
  Virtual_Button(Button *button1, Button *button2,
                 TaskFunction_t on_single_press, TaskHandle_t &task_handle_but1,
...
struct Button {
  ...
  TaskFunction_t OnSinglePressTaskFunction, OnDoublePressTaskFunction,
      OnLongPressTaskFunction;

  Button(uint32_t button_pin, TaskFunction_t on_single_press,
         TaskFunction_t on_double_press, TaskFunction_t on_long_press)
      : pin(button_pin),
  ...
  void OnTimerIterrupt(TaskHandle_t &OnTimerTaskHandle) {
    ...
    // Обрабочкик двойного зажатия кнопки
    // _-_----*
  ...
  void Set_Parent(Virtual_Button *button_parent) { parent = button_parent; }
};

Virtual_Button::Virtual_Button(Button *button1, Button *button2,
